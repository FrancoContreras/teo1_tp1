...
/* constantes */
    ...
    //CTE_INT = {DIGITO}+
    //CTE_FLT =
    //CTE_STRING  = “({DIGITO}|{ALFABETICO}|{ESPECIAL}|{ARITMETICO}|{LOGICO})*"
    //CTE_HEX = 0h({DIGITO}|{A}|{B}|{C}|{D}|{E}|{F})+
...
/* VARIOS */
    ...
    "0h({DIGITO}|{A}|{B}|{C}|{D}|{E}|{F})+"                         {return new Token("CTE_HEX", yytext());}
    {DIGITO}+                                                       {return new Token("CTE_INT", yytext());}
    {ALFABETICO}({DIGITO}|{ALFABETICO})*                            {return new Token("ID", yytext());}
}
